...
def update_reservation(reservation_id):
    ...
    if current_user.is_admin or reservation.user_id == current_user.id:
        ...
        # Ensure end_time is within 1 hour of start_time
        max_end_time = (datetime.combine(date.min, reservation.time_of_reservation) + timedelta(hours=1)).time()
        if reservation.end_time_of_reservation > max_end_time:
            flash('End time must be within 1 hour of start time.')
            return redirect(url_for('booking'))
        if reservation.end_time_of_reservation <= reservation.time_of_reservation:
            flash('End time must be after start time.')
            return redirect(url_for('booking'))
        db.session.commit()
